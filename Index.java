...
public class Index {
	...
	public static void main(String[] args) throws IOException {
		...
		for (File block : dirlist) {
			...
			HashMap<Integer, PostingList> finalmap = new HashMap<Integer, PostingList>();
			/* For each file */
			for (File file : filelist) {
				...
				while ((line = reader.readLine()) != null) {
					...
					for (String token : tokens) {
						if (termDict.containsKey(token) != true ){
							termDict.put(token,wordIdCounter++) ;
						}
						int termId = termDict.get(token); //gets the termid by having token as the key.
						ArrayList<Integer> list1 = new ArrayList<Integer>();
						list1.add(docDict.get(fileName));
						PostingList posting = new PostingList(termId, list1);
						if (finalmap.containsKey(termDict.get(token)) == false ){
								finalmap.put(termId,posting);
						}
						else{
							list1.add(docDict.get(fileName));
							finalmap.put(termId,posting);
						}
						//Now that we have populated termDict, we need to populate postingList
